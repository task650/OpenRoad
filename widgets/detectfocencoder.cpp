...
{
    if (mOpenroad) {
        HelpDialog::showHelp(this, mOpenroad->infoConfig(), "help_foc_encoder_detect");
    }
}
...
void DetectFocEncoder::on_startButton_clicked()
{
    if (mOpenroad) {
        ...
        if (reply == QMessageBox::Ok) {
            mOpenroad->commands()->measureEncoder(ui->currentBox->value());
        }
    }
}
...
{
    if (mOpenroad) {
        mOpenroad->mcConfig()->updateParamDouble("foc_encoder_offset", ui->offsetBox->value());
        mOpenroad->mcConfig()->updateParamDouble("foc_encoder_ratio", ui->ratioBox->value());
        mOpenroad->mcConfig()->updateParamBool("foc_encoder_inverted", ui->invertedBox->currentIndex() != 0);
        mOpenroad->emitStatusMessage(tr("Encoder Parameters Applied"), true);
    }
}

OpenroadInterface *DetectFocEncoder::openroad() const
{
    return mOpenroad;
}

void DetectFocEncoder::setOpenroad(OpenroadInterface *openroad)
{
    mOpenroad = openroad;

    if (mOpenroad) {
        connect(mOpenroad->commands(), SIGNAL(encoderParamReceived(double,double,bool)),
...
{
    if (offset > 1000.0) {
        mOpenroad->emitStatusMessage(tr("Encoder not enabled in firmware"), false);
        QMessageBox::critical(this, "Error", "Encoder support is not enabled. Enable it in the general settings.");
    } else {
        mOpenroad->emitStatusMessage(tr("Encoder Result Received"), true);
